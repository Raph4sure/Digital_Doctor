        ...
        try {
            ...
            if (response.ok) {
                ...
                if (result.error.includes('email')) {
                   const emailError =  document.querySelector('#email').closest('.form_val').querySelector('.error');
                    emailError.textContent = result.error;
                } else if (result.error.includes('password')) {
                    const passwordError = document.querySelector('#password').closest('.form_val').querySelector('.error');
    ...
    formElement.addEventListener("submit", async (event) => {
        ...
        try {
            const response = await fetch(
                "http://localhost:3300/api/patients/login",
                {
                    method: "POST",
